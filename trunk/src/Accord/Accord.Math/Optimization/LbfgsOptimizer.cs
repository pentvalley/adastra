...
namespace Accord.Math.Optimization
{
    ...
    /// </remarks>
    /// 
    public class LbfgsOptimizer : IOptimizer
    {
        ...
        public LbfgsOptimizer(int parameters)
        {
            if (parameters <= 0)
        ...
        public LbfgsOptimizer(int parameters, Func<double[], double> function, Func<double[], double[]> gradient)
            : this(parameters)
        {
            if (function == null)
        ...
        public LbfgsOptimizer(int parameters, Func<double[], double> function, Func<double[], double[]> gradient, Func<double[]> diagonal)
            ...
            ref double stp, out int nfev, double[] wa)
        {
            ...
            while (true)
            {
                ...
                if (stp == stpmax && f <= ftest1 && dg <= dgtest)
                    throw new LineSearchFailedException(5, "The step is at the upper bound defined by 'stpmax'.");

                if (stp == stpmin && (f > ftest1 || dg >= dgtest))
                    throw new LineSearchFailedException(4, "The step is at the lower bound defined by 'stpmin'.");
