...
EegAcq = synchro_GazeEEG_createDataFilter('EegAcq',EegAcq,'Notch',50,'HP',2,'feedback',1);
...
%% Set channel names
synchro_GazeEEG_setChannelNamesMontageParis();
...
%NewEpochs = synchro_GazeEEG_SelectEpochs_5_epochs ... % Warning used only with 5 epochs !!!!!!!!
NewEpochs = synchro_GazeEEG_SelectEpochs ...
                          ('EpochEvents',EpochEvents, ...
                           'TimeInterval', TimeInterval, ...
                           'MaxEpochsToProcess',MaxEpochs, ...
                           'StartFromTrigger', EegAcq.Events.EventTypes.trigger130); % process all triggers after "Learn" phase - trigger130

%% Read data from file, apply filter if specified, fill EEG structure
synchro_GazeEEG_BuildEpoch('FileNameRawData',filename_egg_raw, ...
                           'TimeInterval', TimeInterval, ...
                           'Epochs',NewEpochs, ...
                           'FilterData',true, ...
                           'DispEvents', DisplayEvents, ...
                           'NbNonEEGChan', 5, ... %non EEG channels such as 'EOGV','EOGH'
                           'MaxEpochsToProcess',MaxEpochs);
...
EEG.setname = 'Loaded w/o filtering';
